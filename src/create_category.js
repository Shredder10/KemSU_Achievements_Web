  ...
  return(
    <div className="App">
      ...
      <div className="App-main-full">
        <p className="Header">Создание категории</p>
        <CreateRewardPage />
      </div>
    </div>
  )
...
function CreateRewardPage() { 
  ...
  return (
    <div>
      <p className="Name">
        Категория
        <input className="Input" id="name" type="text" placeholder="Категория" />
      ...
      <p><button hidden={false} id="create_button" className="Button" onClick={function(){Create()}}>Создать категорию</button></p>
    </div>  
  )
}
...
async function Create() {
    ...
    else tmp["categoryName"]=document.getElementById("name").value;
    ...
    let request = await CreateCategory(tmp);
    if(request.ok) {
